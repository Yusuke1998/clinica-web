...
return ContactForm::newContactForm('main_contact_form')
    ...
    ->sendTo(Configuration::first()->email)
    ...
    ->addFields(
        ...
        [
            ...
            ->setLabelName("Tu Nombre") // the <label>$label_name</label> value
...
//                        ->setAsFromName() // if you want the email to set the 'from name' to this value
            ...
            Email::newNamed("email") // the field name <input name='email'>
                ->setLabelName("Tu correo electronico") // the <label>Your email address</label>
                ->setPlaceholderValue("yusuke@example.com") // the default placeholder value (<input placeholder=$placeholder>)
...
//                ->max(100), // max length
            ...
            Textarea::newNamed("message")
                ->setLabelName("Tu mensaje")
//                        ->defaultValue("I wanted to get in touch to ask about...")
                ->setDescription("Porfavor, intenta detallar lo mejor posible tu mensaje") // an optional bit of text that is displayed below the field
                ->markAsRequiredField()
                ->max(5000),
...
//                    'sidebar_section_title'=>"Welcome to our contact page",
    ...
    ->setHtmlAboveAndBelowForm(
        '<div style="text-align:center;"><h3>Contactanos</h3><p>Puedes disponer del formulario de contacto abajo, para comunicarte con nosotros! </p></div>', '')
    ...
    ->setSubmitButtonTextAndClasses("Enviar!", "btn btn-primary")
   ;
